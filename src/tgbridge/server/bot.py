...
def build_dispatcher(allowed_ids: set[int]) -> Dispatcher:
    ...
    async def on_id(msg: Message) -> None:
        ...
        allowed = "да" if uid in allowed_ids else "НЕТ — добавь в TGBRIDGE_ALLOWED_USER_IDS"
        await msg.answer(f"твой id: `{uid}`\nв allowlist: {allowed}", parse_mode="Markdown")

    @dp.message(Command("ping"))
    async def on_ping(msg: Message) -> None:
        await msg.answer("pong")

    # основной обработчик: только разрешённые пользователи, только текст
    @dp.message(F.text & ~F.text.startswith("/"))
    async def on_text(msg: Message, db: DB, wake: asyncio.Event) -> None:
        uid = msg.from_user.id if msg.from_user else 0
        if uid not in allowed_ids:
            log.warning("отклонён промпт от чужого id=%s", uid)
            await msg.answer("не в allowlist, игнорирую")
            return
        cmd_id = db.enqueue(prompt=msg.text, chat_id=msg.chat.id)
        wake.set()  # разбудить висящий long-poll агента
        await msg.answer(f"⏳ принято, задача #{cmd_id}")
