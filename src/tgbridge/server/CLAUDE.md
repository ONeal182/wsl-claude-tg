...
- **Уведомление о результате:** при непустом `session_id` в текст добавляется строка `🧩 сессия <id>` и инлайн-кнопка `resume_keyboard(session_id)` (▶️ Перейти к сессии, `callback_data="resume:<id>"`). Колбэк ловит хендлер `on_resume_cb` (`F.data.startswith("resume:")`) → `resume_reply()` → `cb.answer()` тостом. Кнопка = тот же эффект, что `/resume <id>`.
